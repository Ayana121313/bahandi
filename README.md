...
If `ROBOFLOW_API_KEY` is empty, the backend returns a local demo workflow payload so the UI remains usable for review. A real Roboflow call is made only from `services/roboflow`.
...
The smoke test calls the backend Roboflow service with a tiny generated sample image and verifies that workflow output keys and parsed fields exist. Without `ROBOFLOW_API_KEY`, it runs against the local demo workflow payload. With `ROBOFLOW_API_KEY`, it performs a live hosted workflow call.
